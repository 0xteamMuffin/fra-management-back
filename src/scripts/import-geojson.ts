...
async function importDistricts(stateId: string) {
  try {
    ...
    console.log(
      `📊 Found ${geoData.features.length} districts in GeoJSON file`,
    );
    ...
    for (const feature of geoData.features as DistrictFeature[]) {
      ...
      try {
        ...
        console.error(
          `❌ Failed to import district ${districtName}:`,
          districtError,
        );
      }
    }
    ...
    console.log(
      `📈 Districts import summary: ${importedCount} imported, ${skippedCount} skipped`,
    );
  } catch (error) {
...
async function importVillages(stateId: string) {
  try {
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      const batch = geoData.features.slice(
        startIndex,
        endIndex,
      ) as VillageFeature[];
      ...
      for (const feature of batch) {
        ...
        try {
          ...
          console.error(
            `❌ Failed to import village ${villageName}:`,
            villageError,
          );
          errorCount++;
        }
    ...
    console.log(
      `📈 Villages import summary: ${importedCount} imported, ${skippedCount} skipped, ${errorCount} errors`,
    );
  } catch (error) {
